...
vm.runInContext(fs.readFileSync(root+'/dist/evidence.js','utf8'),context);
assert.match(elements.get('evidence').innerHTML,/55\.9%/);
assert.match(elements.get('evidence').innerHTML,/24\.3%/);
assert.match(elements.get('evidence').innerHTML,/75%/);
assert.match(elements.get('evidence').innerHTML,/Practical recommendations/);
assert.doesNotMatch(elements.get('evidence').innerHTML,/NaN|undefined|Infinity/);
